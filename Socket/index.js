...
// Chỉ gọi dotenv một lần với đường dẫn đúng tới file .env
require('dotenv').config({ path: '../server/.env' });

// Bật chế độ debug của Mongoose
...
// Kết nối MongoDB với các tùy chọn đầy đủ
...
db.once('open', async () => {
  ...
  // Import các mô hình sau khi đã kết nối
  const messageModel = require("../Server/Models/messageModel"); 
  const chatModel = require("../Server/Models/ChatModel"); // Nếu cần sử dụng

  // Thử lưu một tài liệu mẫu để kiểm tra kết nối và mô hình


  // Tiếp tục khởi chạy socket server
  const io = new Server({
    cors: {
  ...
  io.listen(3000); // Chạy server trên cổng 3000
});
